#!/usr/bin/env python3
"""
Super-Twisting Sliding Mode Controller (ST-SMC) for WAM-V (ROS2, Python)

- Sliding variable: s = yaw_err + lambda_ct * cross_track
- Super-twisting control supplies smoothing switching:
    u_st = -k1*sqrt(|s|)*sign(sat_s) - v
    v_dot = -k2 * sign(sat_s)
  where sat_s = sat(s/epsilon_st) used to smooth the sign for noisy measurements.
- r_cmd = -k_psi*yaw_err + u_st (map to moment with k_moment)
- Thruster mapping identical to previous SMC node.
"""
...
class STSMCWamV(Node):
    def __init__(self):
        super().__init__('st_smc_control')

        # --- parameters (defaults you can change) ---
        ...
        # controller gains (ST-SMC specific)
        self.declare_parameter('desired_speed', 0.4)
        self.declare_parameter('k_speed', 80.0)
        self.declare_parameter('k_psi', 5.0)      # P term on yaw_err
        self.declare_parameter('k_moment', 4.0)   # maps r_cmd to moment

        # super-twisting gains
        self.declare_parameter('k1', 10.0)         # sqrt term gain
        self.declare_parameter('k2', 5.5)         # integrator gain

        # smoothing / sat for ST integrator
        self.declare_parameter('epsilon_st', 0.2)  # saturation scaling for sign smoothing

        # SMC geometry
        self.declare_parameter('lambda_ct', 1.0)   # cross-track weight
        # lookahead
        self.declare_parameter('lookahead_distance', 0.4)

        # SMC safety parameters
        self.declare_parameter('r_cmd_limit', 10.0)
        self.declare_parameter('M_cmd_limit', 80.0)
        ...
        self.declare_parameter('executed_marker_topic', '/st_smc/executed_path_marker')
        ...
        self.k1 = float(self.get_parameter('k1').get_parameter_value().double_value)
        self.k2 = float(self.get_parameter('k2').get_parameter_value().double_value)
        self.epsilon_st = float(self.get_parameter('epsilon_st').get_parameter_value().double_value)
        ...
        self.r_cmd_limit = float(self.get_parameter('r_cmd_limit').get_parameter_value().double_value)
        self.M_cmd_limit = float(self.get_parameter('M_cmd_limit').get_parameter_value().double_value)
        ...
        # super-twisting internal integrator v
        self.v_st = 0.0
        ...
        self.target_pub = self.create_publisher(PoseStamped, '/st_smc/target_point', qos)
        self.marker_pub = self.create_publisher(Marker, '/st_smc/control_marker', qos)
        ...
        self.create_service(Empty, '/st_smc/clear_executed_path', self.clear_executed_path_cb)
        ...
        self.get_logger().info('ST-SMC WAM-V controller started.')

        # debug publishers
        self.pub_s = self.create_publisher(Float64, '/st_smc/debug/s', 10)
        self.pub_v = self.create_publisher(Float64, '/st_smc/debug/v_st', 10)
        self.pub_ust = self.create_publisher(Float64, '/st_smc/debug/u_st', 10)
        self.pub_r_cmd = self.create_publisher(Float64, '/st_smc/debug/r_cmd', 10)
        self.pub_M_cmd = self.create_publisher(Float64, '/st_smc/debug/M_cmd', 10)
        self.pub_left_cmd = self.create_publisher(Float64, '/st_smc/debug/left_cmd', 10)
        self.pub_right_cmd = self.create_publisher(Float64, '/st_smc/debug/right_cmd', 10)
        self.pub_cross_track = self.create_publisher(Float64, '/st_smc/debug/cross_track', 10)
        self.pub_yaw_err = self.create_publisher(Float64, '/st_smc/debug/yaw_err', 10)
        self.pub_F_total = self.create_publisher(Float64, '/st_smc/debug/F_total', 10)
        self.pub_desired_speed = self.create_publisher(Float64, '/st_smc/debug/desired_speed', 10)
    ...
    def path_cb(self, msg: Path):
        ...
        if len(pts) >= 2:
            ...
            self.get_logger().debug(f"Received reference path with {len(pts)} pts")
        else:
    ...
    def odom_cb(self, msg: Odometry):
        ...
        self.current_yaw = wrap_to_pi(raw_yaw)
    ...
    # ---------------- utility: projection + lookahead (same as before) ----------------
    ...
    def control_loop(self):
        ...
        # --- Super-Twisting algorithm (discrete approx) ---
        # smoothed sign (sat) to avoid high-frequency sign switching
        sat_s = sat(s / max(1e-6, self.epsilon_st))
        # update v integrator: v_dot = -k2 * sign(sat_s)  (we use sat for robustness)
        v_dot = - self.k2 * sign(sat_s)
        self.v_st += v_dot * dt
        # u_st = -k1 * sqrt(|s|) * sign(s) - v
        s_sqrt = math.sqrt(max(abs(s), 1e-8))
        u_st = - self.k1 * s_sqrt * sign(s) - self.v_st

        # desired yaw-rate-like command (blend P + ST)
        r_cmd = - self.k_psi * yaw_err + u_st

        # clamp r_cmd and compute M_cmd
        if r_cmd > self.r_cmd_limit: r_cmd = self.r_cmd_limit
        if r_cmd < -self.r_cmd_limit: r_cmd = -self.r_cmd_limit
        M_cmd = self.k_moment * r_cmd
        if M_cmd > self.M_cmd_limit: M_cmd = self.M_cmd_limit
        if M_cmd < -self.M_cmd_limit: M_cmd = -self.M_cmd_limit
        ...
        # publish control marker (arrow) indicating M_cmd
        mk = Marker()
        ...
        # debug publishes
        msg = Float64(); msg.data = float(s); self.pub_s.publish(msg)
        msg.data = float(self.v_st); self.pub_v.publish(msg)
        msg.data = float(u_st); self.pub_ust.publish(msg)
